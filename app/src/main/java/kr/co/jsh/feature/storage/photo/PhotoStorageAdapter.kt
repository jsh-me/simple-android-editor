...
class PhotoStorageAdapter(val click:( Int) -> Unit, private val imageResult : ArrayList<File>, private var context: Context)
    ...
    override fun onCreateViewHolder(
        ...
        return ViewHolder(binding, binding.resultImageStorageView)
    }

    override fun getItemCount(): Int = imageResult.size

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        Glide.with(context).load(imageResult[position]).into(holder.resultImageView)
        holder.resultImageView.setOnClickListener { click(position) }
    }
