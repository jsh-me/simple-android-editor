...
class PhotoPresenter(
    override var view: PhotoContract.View,
    ...
    override fun preparePath(extraIntent: Intent) {
        extraIntent.let {
            val path = it.getStringExtra(Consts.EXTRA_PHOTO_PATH) ?: ""
            path
        }.let {
            view.setPhotoView(Uri.parse(it.addFile()).toFile())
        }
    }
    ...
    override fun uploadFile(uri: String, type: String) {
        ...
        val request = MultipartBody.Part.createFormData(
            "file",
            path,
            RequestBody.create(MediaType.parse("image/*"), Uri.parse(path).toFile())
        )
        ...
        postFileUploadUseCase.postFile(request)
            .subscribe({
                if (it.status.toInt() == 200) {
                    ...
                    sendImageResultToServerWithInfo(
                        PidClass.imageMaskObjectPid,
                        PidClass.imageObjectPid
                    )
                } else {
                    view.uploadFailed(it.message)
                    UserObject.ResponseCode = it.status.toInt()
